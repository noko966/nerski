class Skinner {
  ...
  fillSwatches() {
      let res = [];
      const accent = this.skin.accentBg;
      const body = this.skin.bodyBg;
      const monochromaticA = tinycolor(accent).monochromatic();
      const monochromaticB = tinycolor(body).monochromatic();
      const analogousA = tinycolor(accent).analogous();
      const analogousB = tinycolor(body).analogous();

      res = [
          ...monochromaticA.map((c) => c.toHexString()),
          ...monochromaticB.map((c) => c.toHexString()),
          ...analogousA.map((c) => c.toHexString()),
          ...analogousB.map((c) => c.toHexString()),
      ];
  ...
  createPickerAndTrigger(parent, color, opacity) {
      const swatches = this.fillSwatches();
  ...
  createControl(params) {
      ...
      labelWrapper.className = "sk_checkbox_wrapper";
      ...
      let isCustomTextControl, isCustomTextChb, isCustomTextPckr;
      {
          isCustomTextControl = this.createDiv(
              "sk_checkbox_wrapper state_delay_3"
          );
      ...
      if (isCustomAccentCb && customAccentCb) {
          customAccentControl = this.createDiv(
              "sk_checkbox_wrapper state_delay_4"
          );
      ...
      if (isCustomBorderCb && customBorderCb) {
          borderControl = this.createDiv(
              "sk_checkbox_wrapper state_delay_5"
          );
