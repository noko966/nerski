...
<path d="M7.5,13.5h8c0.6,0,1-0.4,1-1v-8c0-0.6-0.4-1-1-1h-8c-0.6,0-1,0.4-1,1v8C6.5,13.1,6.9,13.5,7.5,13.5z M13.5,16.5
    ...
    mergeConfig(config) {
        ...
        _mergedConfig.overlay.Background.color = tinycolor(_mergedConfig.overlay.Background.color).setAlpha(0.5).toRgbString()
    ...
    generateBackgrounds(essence) {
        ...
        */
        ...
        if (this.variant === 'casino') {
            ...
            this.skin[`${_vb.name}Shadow`] = tinycolor.mix(mixTint, this.skin[_vb.nameBg], 60).setAlpha(this.defaults.alpha.bg)
                .toRgbString();
            this.skin[`${_vb.name}ShadowFade`] = tinycolor.mix(mixTint, this.skin[_vb.nameBg], 60).setAlpha(0)
                .toRgbString();
        }
    ...
    generateTextss(essence) {
        ...
        if (_isCustomTextActive) {
            ...
            essenceTxt = guessVisibleColor(tinycolor(this.skin[_vb.nameBg]).toHexString());
        }
        this.skin[_vb.nameTxt] = essenceTxt;
        this.skin[_vb.nameTxt2] = tinycolor.mix(essenceTxt, essenceBg, 30).toHexString();
        this.skin[_vb.nameTxt3] = tinycolor.mix(essenceTxt, essenceBg, 50).toHexString();
    }
    ...
    toggleEnabled(coltrol) {
        ...
        disabled ? coltrol.classList.remove('disable') : coltrol.classList.add('disable')
    }
    ...
    updateControlFor(name) {
        ...
        const sliderProperty = this.isOverlay(name) ? this.skin.overlayBlur : this.skin[c.nameRadius];
    ...
    fillGradientSwatches() {
        ...
        baseColors.forEach(bc => {
            ...
            res.push(`linear-gradient(0deg, ${bc} 0%, ${shade} ${midStop}%, ${bc} 100%)`);
        });
    ...
    createHTML() {
        ...
        for (let i = 0; i < this.configOrder.length; i++) {
            ...
            const sliderProperty = this.isOverlay(_essence) ? "overlayBlur" : _vd.nameRadius;
    ...
    handleGPicker(event, essence, onChangeCallback) {
        ...
        const colorStops = [[this.skin[_vd.nameBg], 0], [this.skin[_vd.nameBg3], 1]];
    ...
    createControlsWrapper() {
        ...
        tableHeaders.forEach((h, index) => {
            ...
            switch (h) {
                ...
                case "Background":
                    className =
                        "sk_header_label variant-background";
                    break;
                case "Gradient":
                    className =
                        "sk_header_label variant-gradient";
                    break;
                case "Text":
                    className =
                        "sk_header_label variant-text";
                    break;
                case "Accent":
                    className =
                        "sk_header_label variant-accent";
                    break;
                case "Border":
                    className =
                        "sk_header_label variant-border";
                    break;
                case "Radius":
                    className =
    ...
    addStringAnim() {
        ...
        const setPath = (progress) => {
            const p = `M-4 0 -4 20 Q${width * x} ${20 + progress}, ${width + 4} 20 ${width + 4} 0 ${width + 4} 0`
    ...
    createDownloadButton() {
        ...
        actionsWrapper.appendChild(
            this.createDownloadBtn(name, "variant_cta")
        );
        if (name2) {
            actionsWrapper.appendChild(
    ...
    stylerCreateState(selector, el) {
        ...
        if (props.includes("Bg")) {
            this.stylerSkin[selector].colors.background = computedStyles.getPropertyValue(`--${essence}Bg`);;
        }
        if (props.includes("Txt")) {
            this.stylerSkin[selector].colors.text_1 = computedStyles.getPropertyValue(`--${essence}Txt`);;
        }
        if (props.includes("Txt2")) {
            this.stylerSkin[selector].colors.text_2 = computedStyles.getPropertyValue(`--${essence}Txt2`);;
        }
    }
    ...
    createStylerPickers() {
        ...
        this.stylerControls.colors.bg = this.createColorBox();

        const stylerPicker = document.createElement('div');
        stylerPicker.className = "sk_picker_trigger ";

        const stylerPickerCB = (e) => {
            this.handlePicker(e, null, null, (color) =>
                console.log(color)
            );
        }
        stylerPicker.addEventListener('click', stylerPickerCB);

        this.stylerControls.colors.bg = stylerPicker;

        

        root.appendChild(this.stylerControls.colors.bg);
    }
    ...
    stylerStart() {
        ...
        this.stylerState.editableElements = this.stylerState.target.querySelectorAll("[data-sk]");
