...
class Skinner {
    ...
    modifyKey(key, value) {
        ...
        var isAccentReadableOnBody = tinycolor.isReadable(this.skin['bodyBg'], this.skin['accentBg'], {});
        console.log(isAccentReadableOnBody);
        if (!isAccentReadableOnBody) {
            self.message("'Accent' color is not readable on 'Body' color try to make it brighter", true);
            let timeout = null;
            if (!timeout) {
        ...
        for (let i = 0; i < this.configOrder.length; i++) {
            ...
            this[_vd.nameBg] = this.createControl([
                ...
                (e) => {
                    ...
                    let picker = this.createPickerAndTrigger(e.target.parentElement, this.skin[_vd.nameBg]);
                    picker.show();
                ...
                (e) => {
                    let picker = this.createPickerAndTrigger(e.target.parentElement, this.skin[_vd.nameBg_g]);
                    picker.show();
                    picker.on('change', (color, source, instance) => {
                ...
                (e) => {
                    let picker = this.createPickerAndTrigger(e.target.parentElement, this.skin[_vd.nameTxt]);
                    picker.show();
                    picker.on('change', (color, source, instance) => {
                ...
                (e) => {
                    let picker = this.createPickerAndTrigger(e.target.parentElement, this.skin[_vd.nameAccent]);
                    picker.show();
                    picker.on('change', (color, source, instance) => {
                ...
                (e) => {
                    let picker = this.createPickerAndTrigger(e.target.parentElement, this.skin[_vd.nameBorder]);
                    picker.show();
                    picker.on('change', (color, source, instance) => {
    ...
    createControl(params) {
            ...
            isGradientEnabledPckr;
        {
            ...
            isGradientEnabledPckr = this.createColorBox(pickerGradientColor, 'nik_skinner_control_group_picker', picker2Callback);
        ...
        if (isCustomAccentCb && customAccentCb) {
            ...
            customAccentPckr = this.createColorBox(pickerCustomAccentColor, 'nik_skinner_control_group_picker', customAccentCb);
        ...
        if (isCustomBorderCb && customBorderCb) {
            ...
            borderPckr = this.createColorBox(pickerBorderColor, 'nik_skinner_control_group_picker', customBorderCb);
...
.skinner_custom_chb {
    ...
    cursor: pointer;
    border-radius: 12%;
    transition: border-width 0.8s, border-color 1s, background-color 1s;
}
...
.skinner_custom_chb_label > input[type='checkbox']:checked + .skinner_custom_chb {
    border-width: var(--chbSizeHalf);
    border-color: var(--activeChbBg);
    background-color: var(--activeChbBg);
}
...
.skinner_custom_chb_label > input[type='checkbox'] + .skinner_custom_chb:before {
    content: "";
    position: absolute;
    display: block;
    width: var(--chbSizeArrow);
    height: var(--chbSizeArrow);
    border: var(--chbSizeBorder) solid var(--skinnerBg);
    border-top: 0;
    border-right: 0;
...
function init(){
    window.SkinnerInstance = new Skinner(createCss, colorsCasweb, null, null, "casino");
